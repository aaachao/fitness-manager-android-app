...
public class kechengxiugai extends AppCompatActivity implements View.OnClickListener {
    ...
    private void getPhoto() {
        FishBun.with(kechengxiugai.this)
...
//                    path = imageData.getStringArrayListExtra(Define.INTENT_PATH);
    ...
    private void found() {
        ...
        if (TextUtils.isEmpty(mingcheng)  ) {
            Toast.makeText(mContext, "要查询的项目名称不能为空", Toast.LENGTH_SHORT).show();
            return;
        }
        // 服务端验证
    ...
    private void update() {
        ...
        if (TextUtils.isEmpty(oldmingcheng) ) {
            Toast.makeText(mContext, "要修改的项目名称不能为空", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (yesno==1)
        {
            ...
            String url = Constants.BASE_URL + "Course?method=updatewithoutpic";
            OkHttpUtils
                    .post()
                    .url(url)
                    .addParams("oldmingcheng",oldmingcheng)
